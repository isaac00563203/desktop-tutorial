...
def _downloaddata_fromtqsdk(symbol : str, duration_seconds : int, start_dt : datetime, end_dt : datetime) -> DataFrame:
...
def _readdata(filename : str) -> DataFrame:
...
def get_data(symbol : str = "KQ.m@CFFEX.IC",  # 合约代码
    ...
    if csv_filename == None or csv_filename == "":
        return _downloaddata_fromtqsdk(symbol, duration_seconds, start_dt, end_dt)
    ...
    if not file_exist or reload:
        data = _downloaddata_fromtqsdk(symbol, duration_seconds, start_dt, end_dt)
...
if __name__ == "__main__":
    ...
    # 测试getDataFromTqSdk
    data = get_data("KQ.m@CFFEX.IH", 60, "test.csv", datetime(2023, 11, 10, 9, 0, 0), datetime(2023, 11, 15, 15,0,0), True)
    assert os.path.exists(os.path.join(CACHDIR, "test.csv"))
    assert len(data) > 0

    # 再次测试getDataFromTqSdk
    data = get_data("KQ.m@CFFEX.IH", 60, "test.csv", datetime(2023, 11, 10, 9, 0, 0), datetime(2023, 11, 15, 15,0,0), False)
    assert os.path.exists(os.path.join(CACHDIR, "test.csv"))
    assert len(data) > 0
